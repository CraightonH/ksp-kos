// library for calculating various deltaV related values
@LAZYGLOBAL OFF.

function circularizeManeuverDeltaV {
    parameter ves is ship.
    parameter bod is ship:orbit:body.

    local targetV is sqrt(ves:body:mu/(bod:radius + ves:orbit:apoapsis)). //this is the velocity that we need to be going at AP to be circular
    local apVel is sqrt(((1 - ves:orbit:eccentricity) * bod:mu) / ((1 + ves:orbit:eccentricity) * ves:orbit:semimajoraxis)). //this is how fast we will be going
    return targetV - apVel. // this is the deltaV
}
